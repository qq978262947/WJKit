...
- (void)layoutSubviews {
    ...
    if (_swipeHeaderBarScrollDisabled) {
        _swipeHeaderBar.st_top = _swipeHeaderTopInset + self.barHeight;
    }
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context {
    ...
    if (context == SwipeTableViewItemContentOffsetContext) {
        ...
        if (!_swipeHeaderBarScrollDisabled) {
            BOOL isHiddenNavagatinBar = NO;
            CGFloat newOffsetY      = [change[NSKeyValueChangeNewKey] CGPointValue].y;
#if !defined(ST_PULLTOREFRESH_HEADER_HEIGHT)
            CGFloat topMarginOffset = _swipeHeaderTopInset + _barInset + self.barHeight;
            // stick the bar
            if (newOffsetY < - topMarginOffset) {
                ...
                isHiddenNavagatinBar = NO;
            }
#else
            CGFloat topMarginOffset = _headerInset - _swipeHeaderTopInset - self.barHeight;
            // stick the bar
            if (newOffsetY < topMarginOffset) {
                if (_swipeHeaderView) {
...
#endif
            if ([self.delegate respondsToSelector:@selector(swipeTableView:isHiddenNavagatinBar:)]) {
                [self.delegate swipeTableView:self isHiddenNavagatinBar:isHiddenNavagatinBar];
            }
        }
...
- (void)headerViewDidFrameChanged:(STHeaderView *)headerView {
...
#endif
    _swipeHeaderBar.st_top = fmax(_swipeHeaderTopInset + self.barHeight, headerView.st_bottom);
    ...
    BOOL isHiddenNavagatinBar = NO;
    if (offsetY < - topMarginOffset - self.barHeight) {
        isHiddenNavagatinBar = YES;
    }
//    NSLog(@"offsetY:%f topMarginOffset:%f", offsetY, -topMarginOffset);
    if ([self.delegate respondsToSelector:@selector(swipeTableView:isHiddenNavagatinBar:)]) {
        [self.delegate swipeTableView:self isHiddenNavagatinBar:isHiddenNavagatinBar];
    }
}
...
- (void)st_runAtDealloc:(void(^)(void))deallocBlock {
    if (deallocBlock) {

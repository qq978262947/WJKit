...
@interface TestSwipeTableViewVC () <SwipeTableViewDataSource ,SwipeTableViewDelegate ,UIGestureRecognizerDelegate ,UIViewControllerTransitioningDelegate>
...
@property (nonatomic, strong) UIImage *clearImage; // 透明图片
@property (nonatomic, strong) UIImage *navigationImage; // 道涵图片
@property (nonatomic, strong) UIImage *shadowImage;

@end

@implementation TestSwipeTableViewVC
...
- (UIView *)tableViewHeader {
    if (nil == _tableViewHeader) {
        ...
        _tableViewHeader.frame = CGRectMake(0, 0, kScreenWidth, kScreenWidth * (headerImage.size.height/(headerImage.size.width == 0 ? kScreenWidth : headerImage.size.width)));
...
- (void)getDataAtIndex:(NSInteger)index {
    ...
    switch (index) {
        case 0:
            numberOfRows = _type == STControllerTypeNormal?81:110;
            break;
        case 1:
...
- (void)swipeTableView:(SwipeTableView *)swipeView isHiddenNavagatinBar:(BOOL)isHiddenNavagatinBar {
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    UIColor *navigationColor = [UIColor colorWithWhite:1.0 alpha:0.99];// RGBColor(234, 39, 0);
    CGRect rect = CGRectMake(0, 0, kScreenWidth, 64);
    UIGraphicsBeginImageContext(rect.size);
    CGContextRef context = UIGraphicsGetCurrentContext();
    CGContextSetFillColorWithColor(context, [navigationColor CGColor]);
    CGContextFillRect(context, rect);
    UIImage *img = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    
    self.navigationImage = img;
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    [self swipeTableView:self.swipeTableView isHiddenNavagatinBar:NO];
}
